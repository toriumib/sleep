...
- **6言語対応**: Web版はヘッダーの言語セレクタで日本語・英語・スペイン語・ポルトガル語・インドネシア語・韓国語に切替（UI全文翻訳、名言・Tipsは日英＋他言語は英語表示）。Android版は端末の言語設定が英語ならUIラベルが英語表示（`values-en`）
- **リテンション/バイラル**: デイリーログインボーナス（+10コイン/日）、起床記録のSNSシェアボタン（Web Share API、非対応環境はクリップボード）
- **ASO素材**: `store/play-store-listing.md` にPlay Store用の6言語ストア文面・キーワード戦略・収益化ロードマップ
- **公開手順**: `store/play-store-publishing-guide.md` にPlayストア公開の実践ガイド（データセーフティ回答例・AdMob本番化・app-ads.txt含む）。プライバシーポリシーは `web/privacy.html`
